...
;instrument 1: decision level meter
instr decision_level
  kdecision init 0
  kans OSClisten giOSC, "/decision", "f", kdecision
  kvariables init 1
  kans OSClisten giOSC, "/variables", "f", kvariables

  adecision oscil 1, 200 + ((kdecision * 16000) / kvariables), gisine
  outleta "out", adecision
endin

instr backtrack_level
  aenv expon 3, 0.3, 0.0001
...
instr mixer
  ...
  ain2 inleta "in2"
  ain3 inleta "in3"
  outs ain1 + ain2 + ain3, ain1 + ain2 + ain3
endin
